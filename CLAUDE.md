...
`Dockerfile` 的 `CMD` 已經串好（line 46）：
```sh
npx --workspace @office-colosseum/server prisma migrate deploy \
  && npm run db:seed --workspace @office-colosseum/server \
  || true; node packages/server/src/index.js
```

邏輯為 `migrate && seed || true; node ...`：每次 deploy 都會嘗試 `migrate deploy`+`seed`，失敗（DB 暫時不可達、表已 seeded）也不擋 server 起，因為 `node ...` 在 `;` 後面**不論前面結果都會跑**。`||` 在 `&&` 之後吃掉非零 exit code、`;` 再無條件接 `node`。
...
# 注意：root package.json 沒註冊 smoke，必須加 --workspace 前綴或 cd 進 packages/server 才跑得起來
...
- **Prisma + Alpine 兩處設置已完成 — 改動時別誤刪**：以下兩件事 codebase 都已就位（schema + Dockerfile），但若有人重寫 Dockerfile 或 schema 沒注意，prod 啟動會 502 / `[db] prisma migrate deploy failed`：
  1. `schema.prisma` generator 已加 `binaryTargets = ["native", "linux-musl-openssl-3.0.x"]`（node:22-alpine 是 OpenSSL 3.x，預設只打 1.1 的 binary）
  2. `Dockerfile` builder + runtime 兩 stage 都已 `RUN apk add --no-cache openssl`（不裝的話 prisma engine 會 fallback 到舊 binary 然後 `libssl.so.1.1: not found`）
  改 Dockerfile 或 schema 時請保留這兩條。
